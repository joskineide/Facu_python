...
soma_nat = maybeMonad(lambda num : num if num == 1 else num + soma_nat(num - 1))
...
div = maybeMonad(lambda num, num2 : 0 if num < num2 else 1 + div(num - num2, num2))
...
prim_alg = maybeMonad(lambda num : int(str(num)[0]))

assert(prim_alg(5649) == 5)
assert(prim_alg(7) == 7)
assert(prim_alg(None) == None)
assert(prim_alg('text') == None)
...
prod_lista = maybeMonad(lambda num_list : num_list[0] if len(num_list) == 1 else num_list.pop() * prod_lista(num_list))

assert(prod_lista([1,2,3,4,5,6]) == 720)
assert(prod_lista(1) == None)
assert(prod_lista(["1","2",3]) == None)
assert(prod_lista("[1]") == None)
...
contem_parQ = maybeMonad(lambda num_list : False if len(num_list) == 0 else True if num_list.pop() % 2 == 0 else contem_parQ(num_list))
...
todos_imparesQ = maybeMonad(lambda num_list : True if len(num_list) == 0 else False if num_list.pop() % 2 == 0 else todos_imparesQ(num_list))

assert(todos_imparesQ([1,3,5,7]))
assert(todos_imparesQ([]))
assert(not todos_imparesQ([1,2,3,4,5]))
assert(todos_imparesQ(1) == None)
...
pertenceQ = maybeMonad(lambda num_list, num : False if len(num_list) == 0 else True if num_list.pop() == num else pertenceQ(num_list, num))
...
junta = maybeMonad(lambda num_list, num_list2 : num_list if len(num_list2) == 0 else junta(num_list, num_list2) if num_list.append(num_list2.pop(0)) == None else None)

assert(junta([1,2,3],[4,5,6]) == [1, 2, 3, 4, 5, 6])
assert(junta([],[4,5,6]) == [4, 5, 6])
assert(junta([1,2,3],[]) == [1, 2, 3])
assert(junta([1,2,3], None) == None)
assert(junta([1,2,3], 1) == None)
...
checkPrimo = maybeMonad(lambda num, num2 = 2 : True if num2 >= num else False if num % (num2) == 0 else checkPrimo(num, num2 + 1))
temPrimoQ = maybeMonad(lambda num_list_list : False if len(num_list_list) == 0 else temPrimoQ(num_list_list) if len(num_list_list[0]) == 0 and not num_list_list.pop(0) == None else True if checkPrimo(num_list_list[0].pop(0)) else temPrimoQ(num_list_list))
...
inverteLista = maybeMonad(lambda num_list : num_list if num_list.reverse() == None else num_list)
